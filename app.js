...
$('#file').addEventListener('change', async (e)=>{
  ...
  const text = await new Promise(res=>{ const fr=new FileReader(); fr.onload=()=>res(String(fr.result||'')); fr.readAsText(file,'UTF-8'); });
  function sniffDelimiter(t){
    const first = (t.split(/\r?\n/)[0] || '').trim();
    const cand = [',',';','\t','|'];
    let best = { ch:',', cnt:0 };
    for(const ch of cand){ const re = ch==='\\t' ? /\\t/g : new RegExp(ch,'g'); const cnt=(first.match(re)||[]).length; if(cnt>best.cnt) best={ch,cnt}; }
    return best.ch;
  }
  const delim = sniffDelimiter(text);
  const papaParsed = await new Promise(resolve=>{ Papa.parse(text,{header:true,skipEmptyLines:true,delimiter:delim,complete:r=>resolve(r),error:()=>resolve(null)}); });
  let parsed = papaParsed;
  if(!parsed || !parsed.meta || !(parsed.meta.fields||[]).length){
    const lines = text.split(/\r?\n/).filter(l=>l.trim().length>0);
    if(lines.length){
      const hdrs = lines[0].split(delim).map(s=>s.replace(/^\"|\"$/g,'').trim());
      const data = lines.slice(1).map(line=>{ const cols=line.split(delim).map(s=>s.replace(/^\"|\"$/g,'').trim()); const obj={}; hdrs.forEach((h,i)=>obj[h]=cols[i]); return obj; });
      parsed = { data, meta:{ fields: hdrs, delimiter: delim } };
    }
  }
  if(!parsed || !parsed.meta || !(parsed.meta.fields||[]).length){ if(st) st.textContent='Не успях да прочета CSV.'; return; }
  headers=parsed.meta.fields; rawRows=parsed.data;
  $('#sep').textContent=parsed.meta.delimiter || delim; $('#shape').textContent=`${fmt.format(rawRows.length)}×${headers.length}`;
  if(st) st.textContent=`Успешно заредено. Полета: ${headers.length}, редове: ${rawRows.length}`;
  renderPreview(); fillSelects();
});

function renderPreview(){
  $('#preview thead').innerHTML='<tr>'+headers.map(h=>`<th>${h}</th>`).join('')+'</tr>';
  $('#preview tbody').innerHTML=rawRows.slice(0,8).map(r=>`<tr>${headers.map(h=>`<td>${r[h]}</td>`).join('')}</tr>`).join('');
}
...
$('#btnAnalyze').onclick=()=>{
  ...
  const n=vals.length, mean=vals.reduce((a,b)=>a+b,0)/Math.max(n,1);
